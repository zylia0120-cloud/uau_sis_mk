...
if __name__ == '__main__':
    ...
    for corr in correlation:
        ...
        for lamb in lamb_list:
            ...
            mmca_mk_a_list1 += [mk_a_list]
            mmca_mk_i_list1 += [mk_i_list]

        with open("0_musaeENGB_mmca_mk_a_list_hotmap.txt", "w", encoding="utf-8") as f:
            # 保存为数组形式：[10, 20, 30, 40, 50]
            f.write(str(mmca_mk_a_list1))

        with open("0_musaeENGB_mmca_mk_i_list_hotmap.txt", "w", encoding="utf-8") as f:
            # 保存为数组形式：[10, 20, 30, 40, 50]
            f.write(str(mmca_mk_i_list1))
        ...
        #                                                                mu=mu_, rho_m=rho,alpha=alpha_,max_step=max_step_,
        #                                                                min_step=min_step_, min_err=min_err_)
        #         print(str(beta_u_) + ':mc multiprocess finished!')
        #         i_list_mc_mp += [ave_i_mp]
        #         a_list_mc_mp += [ave_a_mp]
        #
        #     i_list_mc_mp1 += [i_list_mc_mp]
        #     a_list_mc_mp1 += [a_list_mc_mp]
        #
        # with open("0_facebook_mc_mk_a_list_hotmap.txt", "w", encoding="utf-8") as f:
        #     # 保存为数组形式：[10, 20, 30, 40, 50]
        #     f.write(str(a_list_mc_mp1))
        #
        # with open("0_facebook_mc_mk_i_listhotmap.txt", "w", encoding="utf-8") as f:
        #     # 保存为数组形式：[10, 20, 30, 40, 50]
        #     f.write(str(i_list_mc_mp1))
